// Builds and runs scripts/verify-backfill.ts using the same esbuild setup as build.mjs
...
await build({
  entryPoints: [path.join(dir, "verify-backfill.ts")],
  platform: "node",
  bundle: true,
  format: "esm",
  outdir,
...
const res = spawnSync("node", [path.join(outdir, "verify-backfill.mjs")], {
  stdio: "inherit",
});
