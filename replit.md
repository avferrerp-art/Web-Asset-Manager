...
- **Sync de Odoo**: `src/lib/odooClient.ts` habla JSON-RPC con Odoo (config vía secretos `ODOO_*`; si faltan, retorna null y el sync se salta en silencio). `src/services/odooSync.ts` importa cotizaciones y órdenes (`state in [draft, sent, sale, done]`) como filas de `sales`, calculando peso/volumen desde líneas y productos; idempotente por `odoo_id` unique + `onConflictDoNothing`. `odooEstado` espeja el estado comercial sin modificar los propietarios de `estado` y `estadoEntrega`. `GET /sales` solo incluye cotizaciones con `includeQuotations=true`; por omisión admite sale/done/NULL. Corre por polling cada 5 min (primer run ~10s tras boot) y registra el resultado en la tabla `odoo_sync_state` (una sola fila).
